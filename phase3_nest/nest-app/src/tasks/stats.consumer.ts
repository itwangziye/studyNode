...
@Injectable()
export class StatsConsumer implements OnApplicationBootstrap {
    ...
    async onApplicationBootstrap() {
        ...
        const q = await channel.assertQueue("stats_queue", {durable: true})
        ...
        await channel.consume(q.queue, (message) => {
           ...
           this.logger.log(`stats收到消息: ${JSON.stringify(data)}`)
           channel.ack(message)
        })
    }
}