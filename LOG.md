...
## Day 25(2026-08-18)——开工抽考 2过2挂 + 关55上半场:健康检查

### 开工抽考(4题:2过2挂)

| 知识点 | 判定 | 备注 |
|---|---|---|
| 关50 延迟双删 | ✅ L1→L2 | "忘记了"次日回血,T1~T5全链复述对——间隔重复起效的活样本 |
| 关46 bool四子句 | ✅ L1→L2 | 语义全过;但DSL顺手手滑(match裸奔+引号自相矛盾),JSON DSL毕业打破 |
| 关51 幂等4方案 | ❌ L0留 | **第二次挂**:2.5/4,token完全没说,职责分层附加题没答。补讲:三层防线(入口拦/业务拦/DB拦) |
| 关54 监控铁律 | ❌ L0留 | 首考挂:rate完全忘了(答"查询位置错"),基数爆炸没说出,labelNames机制含糊 |

### 关55上半场:健康检查(Step1/Step2)

- Step1 三连坑:**controller塞providers→404** → **exports放controller→启动炸** → **tap打点漏豁免→污染**。同源:三数组职责(controller管路由/provider管注册/exports管开放)。关54同款404第二次踩。
- Step2 真readiness:并发检查(两个Promise同时起飞+allSettled逐项判up/down)→任一down抛ServiceUnavailableException(503)+checks分诊。
- **超时赛跑**:ioredis断连自动重连,ping卡6.7s→Promise.race(检查vs 1s定时器reject)→实测1.004s。K8s探针timeoutSeconds默认1s,探针不能反噬业务。
- 故障注入坑:`redis-cli shutdown`被brew services(launchd KeepAlive)秒拉活——先`brew services stop`停管理器。
- 追问半对:探针为什么绝不能上鉴权(发探针的是K8s不是人,没有token);@Public+Reflector豁免机制没答出,列为选做。

## Day 26(2026-08-19)——关55下半场:优雅停机(通关 🎓)+ 实验三重坑

### 优雅停机演进(三版实测)

1. **裸奔版**(无任何配置):SIGTERM=立即死,在途请求腰斩。裸Node对照实验证实。
2. **enableShutdownHooks版**:钩子链跑了(ES"连接已关闭"日志),但Nest自动顺序是**先拔依赖再关server**→在途请求断粮,还是腰斩。
3. **手写三步曲版**(最终):接管SIGTERM→server.close(停新连接+等在途排空)→app.close(手动触发钩子链断依赖)→exit(0);兜底10s强退(必须<K8s宽限期30s,否则被SIGKILL枪毙)。**通关画面:SIGTERM第1秒发出,curl 3.005s拿到{"slow":true} 200**。

### 🔥 实验三重坑(今天最贵的工程教训:反常结果先怀疑仪器)

1. **lsof -ti:3000 误杀 curl**:lsof匹配"所有跟3000相关的socket"含curl客户端连接→`kill $(lsof -ti:3000)`把发请求的curl一并SIGTERM(exit 143)。此前所有"curl空手"实验全部作废——不是服务腰斩,是仪器杀人。修正:`lsof -ti:3000 -sTCP:LISTEN`只取监听进程。
2. **代理环境变量**:HTTP_PROXY=127.0.0.1:7890且NO_PROXY没配localhost→curl走Clash。修正:`curl --noproxy '*'`。
3. **手动实验假证据**:手敲命令的curl/kill时间差=手速,慢了→kill迟到→"请求先完成"假象像优雅停机。时序敏感实验必须脚本化控时序。

### 用户钦点重点:服务生命周期全景图(入SPACED_REVIEW,08-20首考)

出生(listen→探针→入名单→流量来)/干活(请求八环:中间件→守卫→管道→拦截器进→控制器→拦截器出→过滤器)/退休(摘流量→SIGTERM→排空→断依赖→兜底)。用户自评"流程模糊"主动要求纳入隔期复习——元认知在线。

### 关55通关追问判定

- 信号三兄弟:SIGTERM(通知,能接)/SIGINT(Ctrl+C,能接)/SIGKILL(kill -9+K8s宽限期,接不住)。用户曾答"两信号只是速度不同"(B选项)→被自己代码实验证伪;曾把K8s的SIGKILL说成SIGINT→纠正;SIGKILL特性"终止进程"只到60分,精确版"不可捕获"明日验证。
- K8s背景:用户抗议"k8s没概念为啥一直考"——教练失误,考题超知识边界。补最小图景(探针/摘流量/宽限期三动作)后吸收良好,复述服务一生全链正确。

### 📈 能力跃迁

| 维度 | 之前 | 现在 |
|---|---|---|
| 服务终局认知 | kill=进程没了 | 完整退休流程:摘流量→排空→断依赖→exit,兜底抢在枪毙前 |
| 实验方法论 | 相信输出 | 反常结果先怀疑仪器(lsof误杀/代理/时序),脚本化控变量 |
| 生产视角 | 接口能跑就行 | 探针/指标/日志/停机挂在服务生命周期时间线上的哪一格,有全景图 |

### 🎯 明天计划(08-20 到期)

- 🔴 首考:服务生命周期全景图(闭卷讲两条线)+ 停机三步曲(SIGKILL精确特性验证)
- 🔴 幂等4方案**第三次考**(再不过升S级每天抽)+ 监控踩坑铁律再考 + JSON DSL纯格式补验
- 关56 开题:性能瓶颈定位实战

### 💬 一句话总结

> 今天最大的认知变化:**"退场也是工程"**——一个服务怎么死(先摘流量、排空在途、断依赖、抢在SIGKILL前体面退)和它怎么活着一样重要;以及实验本身的仪器(lsof/代理/时序)也会撒谎,反常结果先怀疑仪器再怀疑对象。
