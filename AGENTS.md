...
6. **贴近企业实战，拒绝玩具例子**
   - 讲概念、举例子、练业务、设计数据模型和 API，一律按**企业真实在用**的方式来，禁用 `foo`/`bar`/`baz`/`xxx`/`hello world` 这类纯演示。
   - 业务领域优先选真实场景：电商订单、文章 CMS、权限 RBAC、支付幂等、秒杀限流——这些是面试和工作中真正会遇到的问题。
   - 技术选型和代码结构对标企业主流做法：讲 JWT 就结合登录态/刷新 token/单点登录的真实链路；讲缓存就讲缓存击穿/雪崩/一致性这些生产真问题；讲并发就讲库存超卖/重复下单，而不是只演示 API 怎么调。
   - 每个例子要带「企业里为什么这么干」：为什么订单要幂等键、为什么列表要分页、为什么敏感字段要脱敏——讲透工程权衡，而不是停留在"能跑就行"。
   - **原因**：练成肌肉记忆的东西，只有在企业里用得上才有价值。玩具例子练熟了，到了工作里照样不会写真实业务。
...
| 用 `foo/bar`/玩具例子讲概念 | 用企业真实业务场景（订单/权限/幂等/限流…）举例 |
